...
//	after this.
//
static BOOL ExpandTabs(USPDATA* uspData, WCHAR* wstr, int wlen, SCRIPT_TABDEF* tabdef)
{
    int i;
    ...
    int charWidth = uspData->uspFontList ? uspData->uspFontList[0].tm.tmAveCharWidth : uspData->defaultFont.tm.tmAveCharWidth;
...
static int MergeSimpleScripts(SCRIPT_ITEM* itemList, int itemCount)
{
    ...
    for (i = 0; i < itemCount - 1; i++) {
        ...
        if (propList[itemList[i + 0].a.eScript]->fComplex == FALSE && propList[itemList[i + 1].a.eScript]->fComplex == FALSE) {
...
//
static BOOL BuildMergedItemRunList(
    USPDATA* uspData,
    WCHAR* wstr,
    int wlen,
    ATTR* attrList,
    ...
    while (i < itemCount) {
        ...
        if (attrLen == 0) {
            while (attrPos + attrLen < wlen) {
                ...
                if (attrPos + attrLen < wlen && (attrList[a].font != attrList[a + 1].font || attrList[a].ctrl != attrList[a + 1].ctrl || attrList[a].eol != attrList[a + 1].eol || attrList[a].ctrl || attrList[a].eol))
                    break;
...
static BOOL BuildVisualMapping(
